//Function to filter results for payment methood
function GetRequiredPaymentMethods(pMethods) {
    var methods = ["wechatpayWeb","alipay_wap", "scheme"];
    var paymentMethods = [];
    pMethods.paymentMethods.forEach(element => {
        if (methods.includes(element.type)) {
            paymentMethods.push(element);
        }
    });
    pMethods.paymentMethods = paymentMethods;
    return pMethods;
}
...
function SetupDropin(payload) {
    ...
    .then((paymentMethodsResponse) => {
        const configuration = {
            paymentMethodsResponse: GetRequiredPaymentMethods(paymentMethodsResponse), // The `/paymentMethods` response from the server.
